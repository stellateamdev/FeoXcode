//
//  CreatedActivityTableViewController.swift
//  Flag
//
//  Created by marky RE on 3/19/2560 BE.
//  Copyright © 2560 marky RE. All rights reserved.
//

import UIKit
import Firebase
class CreatedActivityTableViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        refresh.addTarget(self, action: #selector(CreatedActivityTableViewController.reloadData), for: .valueChanged)
        refresh.tintColor = UIColor.stellaPurple()
        NotificationCenter.default.addObserver(self, selector: #selector(CreatedActivityTableViewController.reloadData), name: NSNotification.Name(rawValue: "reloadTableCreated"), object: nil)
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "CreatedActivityCell", for: indexPath) as! AllActivityTableViewCell
        cell.title.text = createdArray[indexPath.row].title
        cell.time.text = "\(createdArray[indexPath.row].startdateText) - \(createdArray[indexPath.row].enddateText)"
        let num = createdArray[indexPath.row].join.count
    ...
    override func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        if createdArray[indexPath.row].id == currentUser.id {
            view.created = true
        }
    ...
    func queryActivities() {
        let scoresRef = FIRDatabase.database().reference(withPath: "Users/\(FIRAuth.auth()!.currentUser!.uid)/Created")
                ...
                for value in dictionary {
                    ...
                    FIRDatabase.database().reference().child("Activities/\(key)").observeSingleEvent(of: .value, with: { snap in
                        if snap.exists() {
                            ...
                            self.createdArray.append(ActivityData(location: CLLocationCoordinate2D(latitude: Double(dict["latitude"] as! String)!, longitude: Double(dict["longitude"] as! String)!), locationAddress: (dict["locationaddress"] as! String),  startdateText: (dict["startdatetext"] as! String),enddateText: (dict["enddatetext"] as! String), title: dict["title"] as! String, description: (dict["description"] as! String), pictureURL: (dict["thumbnailURL"] as! String),creator:(dict["username"] as! String),id:(dict["uid"] as! String),key:(dict["key"] as! String)))
